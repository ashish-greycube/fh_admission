...
def get_data(filters):
	...
	data = frappe.db.sql("""
		SELECT
			...
			IFNULL(SUM(CASE WHEN lead.lead_status IN ("Replied", "Converted", "Confirmed", "Admission Form Fee Paid", "Contacted", "Interested", "Not Interested", "To Follow-up", "Waitlist", "Parent Refusal", "Admission Withdrawal", "Refused", "Lead") THEN 1 ELSE 0 END), 0) AS leads_contacted,
			IFNULL((COUNT(DISTINCT lead.lead_name) - SUM(CASE WHEN lead.lead_status IN ("Replied", "Converted", "Confirmed", "Admission Form Fee Paid", "Contacted", "Interested", "Not Interested", "To Follow-up", "Waitlist", "Parent Refusal", "Admission Withdrawal", "Refused", "Lead") THEN 1 ELSE 0 END)), 0) AS not_contacted,
			IFNULL((SUM(CASE WHEN (lead.lead_status IN ("Lead", "Interested", "Waitlist", "Converted", "Confirmed", "Admission Form Fee Paid")) THEN 1 ELSE 0 END)) / COUNT(DISTINCT lead.lead_name), 0) AS conversion_rate,
